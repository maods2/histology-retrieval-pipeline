...
if __name__ == "__main__":
    ...
    for root, subdirs, files in os.walk(base_path):
        for trainfile in files:
            ...
            path_test = os.path.join(root, testfile)
            path_train = os.path.join(root, trainfile)

            x_test, y_test, _ = load_embeddings(path_test)
            x_train, y_train, _ = load_embeddings(path_train)

            # Normalize embeddings 
            x_train = normalize_with_L2_norm(x_train)
            x_test = normalize_with_L2_norm(x_test)
            
            m_name = trainfile.replace("_train.pickle","")
    ...
    MODELS = ['autoenconder',  'semant_att', 'semant_autoenconder','semant_triplet', 'semant_swav', 'swav','triplet']
    METRICS = ['mAP@']
    DISTANCES = ['euclidian', 'cosine similarity']
